...
describe('trackerFor', () => {
  it('counts the connecting address when no key is configured', () => {
    const request = {
      ip: '10.0.0.5',
      headers: { 'x-da-internal': KEY, 'x-da-client-ip': '1.2.3.4' },
    };
  ...
  it('ignores a forwarded address sent without the key, or with the wrong one', () => {
    expect(trackerFor({ ip: '9.9.9.9', headers: { 'x-da-client-ip': '1.2.3.4' } }, KEY)).toBe(
      '9.9.9.9',
    );
    expect(
      trackerFor(
        { ip: '9.9.9.9', headers: { 'x-da-internal': 'guess', 'x-da-client-ip': '1.2.3.4' } },
        KEY,
      ),
    ).toBe('9.9.9.9');
  });
  ...
  it('ignores a forwarded value that is not an address', () => {
    const request = {
      ip: '10.0.0.5',
      headers: { 'x-da-internal': KEY, 'x-da-client-ip': 'anything' },
    };
  ...
  it('accepts IPv6', () => {
    const request = {
      ip: '10.0.0.5',
      headers: { 'x-da-internal': KEY, 'x-da-client-ip': '2001:db8::1' },
    };
